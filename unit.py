...
class BaseUnit(ABC):
    ...
    def health_points(self):
        """ returning health level"""
        return round(self.hp, 1)

    @property
    def stamina_points(self):
        """ returning stamina level """
        return round(self.stamina, 1)

    def equip_weapon(self, weapon: Weapon):
        """new weapon for player"""
        self.weapon = weapon
    ...
    def use_skill(self, target: BaseUnit) -> str:
        """
        skill use method
        if skill already been used it returns message 'skil has already been used'
        """
...
class PlayerUnit(BaseUnit):
    ...
    def hit(self, target: BaseUnit) -> str:
        """
        Player hit function:
        checking if stamina enough for the hit.
